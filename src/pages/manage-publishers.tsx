...
import { Building2, Plus, Search, CheckCircle2, XCircle, MoreVertical, Wallet, Calendar, AlertCircle, Loader2 } from "lucide-react";
...
export default function ManagePublishers() {
  ...
  // Gasless Admin Hook
  const { addPublisher, loading: isAdding, error: addError, result: addResult, step } = useGaslessAdmin();
  ...
  useEffect(() => {
    const fetchPublishers = async () => {
      ...
      try {
        ...
        const uniquePublishers = Array.from(new Map(fetchedPublishers.map(p => [p.wallet, p])).values());
      ...
      >
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="bg-background/95 backdrop-blur-xl border-white/10">
            <DialogHeader>
              <DialogTitle className="text-white">Add New Publisher (Gasless)</DialogTitle>
            </DialogHeader>
        ...
        >
          <GlassCard className="relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-4xl font-bold text-white mt-1">{publishers.length}</h3>
            </div>
          </GlassCard>
        </motion.div>
        {/* Other stats cards could be dynamic too but we lack data points for now, keeping them static or based on publishers length */}
      </div>
